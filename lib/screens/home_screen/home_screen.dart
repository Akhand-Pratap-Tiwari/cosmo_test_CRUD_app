...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _employeeListWidget,
      floatingActionButton: AddEmployeeButton(
        addEmployCallback: () { 
          Navigator.push(   //On tap goes to Employee Add screen
            context,
            MaterialPageRoute(
              builder: (context) => const CreateEmployeeScreen(),
            ))
            .then((_) => setState(() {              //The list will reload after we add the employee and come back
              _employeeListWidget = EmployeesList(  //This is ensured by reassigning global key
                key: GlobalKey(),
              );
            }),
          );
